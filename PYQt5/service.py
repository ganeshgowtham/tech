...
class ChatService:
    ...
    def get_system_health(self):
        """Returns a list of system health check messages with their status and overall health status message"""
        try:
            health_checks = [
                {"message": "Logging system", "status": self._check_logging()},
                {"message": "Command system", "status": self._check_commands()},
                {"message": "Message processing", "status": self._check_message_processing()},
                {"message": "Email system", "status": self._check_email_system()}
            ]
            
            # Check if all statuses are True
            all_checks_passed = all(check["status"] for check in health_checks)
            
            # Add the overall status message
            status_message = "All checks passed, you are good to go" if all_checks_passed else "Health check failed, check admin"
            return {"checks": health_checks, "message": status_message}
            
        except Exception as e:
            logging.error(f"Health check failed: {str(e)}")
            return {"checks": [{"message": "System health check", "status": False}], 
                    "message": "Health check failed, check admin"}

    def _check_logging(self):
        """Check if logging system is working"""
        try:
            logging.info("Health check: Logging system test")
            return True
        except:
            return False

    def _check_commands(self):
        """Check if command system is working"""
        try:
