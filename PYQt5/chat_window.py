...
class ChatWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # Display health check messages in chat console
        self.display_health_check()

    def display_health_check(self):
        """Display system health check messages in the chat console"""
        health_result = self.chat_service.get_system_health()
        windows_user = self.get_windows_username()
        
        # Add header message with Windows username
            ...
            <div style='background-color: #1D4BA3; padding: 10px; border-radius: 8px; margin: 5px 0;
                ...
                <table width="100%">
                    <tr>
                        ...
                        <td><b style='color: #3498db;'>Hi {windows_user}, I am checking health check of various systems:</b></td>
                    </tr>
                </table>
            </div>
        """)
        ...
        for check in health_result["checks"]:
            ...
            status_color = "#2ecc71" if check["status"] else "#e74c3c"
            ...
            self.chat_display.append(f"""
                <div style='background-color: #1D4BA3; padding: 10px; border-radius: 8px; margin: 5px 0;
                    ...
                    <table width="100%">
                        <tr>
                            <td width="40" valign="top">{status_icon}</td>
                            <td><span style='color: {status_color};'>{check["message"]}</span></td>
                        </tr>
                    </table>
                </div>
            """)
        ...
        status_color = "#2ecc71" if all(check["status"] for check in health_result["checks"]) else "#e74c3c"
        self.chat_display.append(f"""
            <div style='background-color: #1D4BA3; padding: 10px; border-radius: 8px; margin: 5px 0;
                 border-left: 4px solid {status_color};'>
                <table width="100%">
                    <tr>
                        <td width="40" valign="top">ℹ️</td>
                        <td><span style='color: {status_color};'>{health_result["message"]}</span></td>
                    </tr>
                </table>
            </div>
        """)
    ...
    def setup_ui(self):
        ...
        # Theme toggle link
        theme_link = QLabel("🌙" if self.is_dark_mode else "☀️")
        theme_link.setProperty('class', 'sidebar-link')
        theme_link.setCursor(Qt.PointingHandCursor)
        theme_link.mousePressEvent = lambda e: self.toggle_theme()
        theme_link.setToolTip("Toggle Theme")
        
        # History link
        history_link = QLabel("📝")
        history_link.setProperty('class', 'sidebar-link')
        history_link.setCursor(Qt.PointingHandCursor)
        history_link.mousePressEvent = lambda e: self.toggle_history()
        history_link.setToolTip("Chat History")
        ...
        notif_link = QLabel("🔔")
        notif_link.setProperty('class', 'sidebar-link')
        notif_link.setCursor(Qt.PointingHandCursor)
        notif_link.setToolTip("Notifications")
        
        self.notif_badge = QLabel("0")
        self.notif_badge.setProperty('class', 'notification-badge')
        self.notif_badge.hide()
        
        notif_layout.addWidget(notif_link)
        notif_layout.addWidget(self.notif_badge, 0, Qt.AlignTop | Qt.AlignRight)
        
        # Log viewer link
        log_link = QLabel("📋")
        log_link.setProperty('class', 'sidebar-link')
        log_link.setCursor(Qt.PointingHandCursor)
        log_link.mousePressEvent = lambda e: self.toggle_logs()
        log_link.setToolTip("View Logs")
        
        # Info link
        info_link = QLabel("ℹ️")
        info_link.setProperty('class', 'sidebar-link')
        info_link.setCursor(Qt.PointingHandCursor)
        info_link.mousePressEvent = lambda e: self.show_contact_info()
        info_link.setToolTip("Information")
        
        # Add links to sidebar
        sidebar_layout.addWidget(theme_link)
        sidebar_layout.addWidget(history_link)
        sidebar_layout.addWidget(notif_container)
        sidebar_layout.addWidget(log_link)
        sidebar_layout.addWidget(info_link)
        sidebar_layout.addStretch()
        
        # Update toggle_theme method to update theme link text
        def toggle_theme(self):
            self.is_dark_mode = not self.is_dark_mode
            self.current_theme = self.DARK_THEME if self.is_dark_mode else self.LIGHT_THEME
            # Find and update theme link text
            for widget in self.findChildren(QLabel):
                if widget.property('class') == 'sidebar-link' and widget.text() in ['🌙', '☀️']:
                    widget.setText("🌙" if self.is_dark_mode else "☀️")
            self.apply_theme()
        
        # Rest of the existing setup_ui code...
    ...
    def apply_theme(self):
        ...
        # Update sidebar style - removed border-right
    ...
    def setup_history_panel(self):
        ...
        close_btn.clicked.connect(lambda: self.close_popup(self.history_widget))
    ...
    def setup_contact_panel(self):
        ...
        close_btn.setFixedSize(20, 20)
        close_btn.setProperty('class', 'close-btn')
        close_btn.clicked.connect(lambda: self.close_popup(self.contact_widget))
    ...
    def setup_log_panel(self):
        ...
        email_btn.setProperty('class', 'email-btn')
        email_btn.clicked.connect(self.send_logs_email)
        
        close_btn = QPushButton("×")
        close_btn.setFixedSize(20, 20)
        close_btn.setProperty('class', 'close-btn')
        close_btn.clicked.connect(lambda: self.close_popup(self.log_widget))
    ...
    def close_popup(self, widget):
        """Generic method to close any popup widget"""
        if widget.isVisible():
